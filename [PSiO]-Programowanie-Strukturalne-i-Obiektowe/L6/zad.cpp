...
template <typename T>
T
operator* (std::vector<T> v1, std::vector<T> v2)
{
  if (v1.size () != v2.size ())
    {
      throw std::out_of_range ("vectors a and b need to be the same size to "
...
public:
  ...
  trygonometric ()
  {
    std::cout << "z= " << magnitude () << "*cos(" << angle () << ") + i*sin("
              << angle () << ")";
  }
  ...
  friend std::ostream &
  operator<< (std::ostream &os, const Complex<T> &complex)
  {
...
public:
  ...
  void
  emplace (const std::vector<T> &data)
  {
  ...
  bool
  from_csv (const std::string &filename, char delim = ',', int column_idx = 4,
            bool headers = true)
  {
  ...
  max () const
  {
    auto max_ = std::max_element (
        bins_.begin (), bins_.end (),
        [] (T_pair a, T_pair b) { return a.second < b.second; });
  ...
  range () const
  {
    auto [min_, max_] = std::minmax_element (
        bins_.begin (), bins_.end (),
        [] (T_pair a, T_pair b) { return a.second < b.second; });
  ...
  static Histogram
  generate (int n, T (*get_random_item) ())
  {
    Histogram H;
    while (n--)
      {
...
public:
  ...
  static Matrix
  fill (int rows, int cols, std::function<T ()> gen_num)
  {
    ...
    if (a.size () != b.size ())
      {
        throw std::out_of_range (
            "the size of matrices A and B must match in expression A+B");
      }
  ...
  operator* (const Matrix<T> &a, const Matrix<T> &b)
  {
    ...
    if (a.size ().second != b.size ().first)
      {
        throw std::out_of_range (
            "the number of column in matrix A must match the "
            "number of rows in matrix B in expression A*B");
      }
